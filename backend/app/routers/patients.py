...
@router.get("/patients")
def get_all_patients(db: DatabaseSession) -> List[PatientResponse]:
    patient_service = PatientService(db)
...
def get_notes_by_patient_id(db:DatabaseSession, patient_id:int) -> List[PatientNoteResponse]:
    note_service = PatientNoteService(db)
    return note_service.get_notes_by_patient_id(patient_id)
...
def get_summary(db:DatabaseSession, patient_int:int) -> PatientSummaryResponse:
    summary_service = PatientSummaryService(db)
    return summary_service.get_patient_summary(patient_int)
